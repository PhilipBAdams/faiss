...
faiss::LevelSelectionMethod StrToLvlSelect(std::string method)
{
    if (!method.compare("Random")) {
        return faiss::LevelSelectionMethod::Random;
    } else if (!method.compare("PriorMax")) {
        return faiss::LevelSelectionMethod::PriorMax;
    } else if (!method.compare("PriorSum")) {
        return faiss::LevelSelectionMethod::PriorSum;
    } else {
        assert(false);
    }
}
...
int main(int argc, char** argv)
{
    ...
    size_t d;
    assert(argc == 7);
    std::string basevec_fname = argv[1];
    std::string qvec_fname = argv[2];
    std::string prior_fname = argv[3];
    std::string gt_fname = argv[4];
    faiss::LevelSelectionMethod SelectionMethod = StrToLvlSelect(argv[5]);
    double multiplier = std::stod(argv[6]);

    {
        printf ("[%.3f s] Loading database\n", elapsed() - t0);

        size_t nb, d2;
        float *xb = fvecs_read(basevec_fname.c_str(), &d2, &nb);
        //assert(d == d2 || !"dataset does not have same dimension as train set");
        size_t np, d3;
        float *xp = fvecs_read(prior_fname.c_str(), &d3, &np);
        index = faiss::index_factory(d2, index_key);
        printf ("[%.3f s] Indexing database, size %ld*%ld\n",
                elapsed() - t0, nb, d2);
        faiss::IndexHNSW* ih = (dynamic_cast<faiss::IndexHNSW*>(index));
        faiss::HNSW& hnsw = ih->hnsw;
        hnsw.lselect = SelectionMethod;
        auto priors = std::vector<float>(xp, xp + np);
        auto dpriors = std::vector<double>(priors.begin(), priors.end());
        for (int i = 0; i < dpriors.size(); i++) {
            dpriors[i] = multiplier*dpriors[i];
        }
        hnsw.priors = dpriors;
        //faiss::PriorSumHNSW isum = faiss::PriorSumHNSW(ih.hnsw);
        //isum.setPriors(std::vector<float>(xp, xp + np));
        //ih.hnsw = isum;
        //ih.add(nb, xb);
        index->add(nb, xb);
...

    {
        ...
        size_t d2;
        xq = fvecs_read(qvec_fname.c_str(), &d2, &nq);
...

    {
        ...
        size_t nq2;
        int *gt_int = ivecs_read(gt_fname.c_str(), &k, &nq2);
